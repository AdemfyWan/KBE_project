...
# CG analysis ----------------------------------------------------------------------------------------------------------
    ...
    @Attribute
    def cog_airframe_x(self):
        x1 = self.body_full.cog[0]
        x2 = self.wing.cog[0]
        vol1 = self.body_full.volume
    ...
    def cg_plot(self):
        ...
        with open(DIR + "/Airfoils/" + self.airfoil_root + ".dat", 'r') as f:
            x_root = []
            z_root = []
            for line in f:
                x, z = line.split(' ', 1)
                x_root.append(float(x) * self.w_c_root)
                z_root.append(float(z) * self.w_c_root * self.t_factor_root)

        with open(DIR + "/Airfoils/" + self.airfoil_tip + ".dat", 'r') as f:
            x_tip = []
            z_tip = []
            for line in f:
                x, z = line.split(' ', 1)
                x_tip.append(float(x) * self.w_c_tip + tip_le_x_pos)
                z_tip.append(float(z) * self.w_c_tip * self.t_factor_tip)

        fig, ax = plt.subplots()

        plt.plot(x_root, z_root, color="gray", linewidth=0.5)
        ...
        plt.plot(self.cog_airframe_x, 0, marker="x", markersize=10, color="red", label="Airframe CG")
...
# Battery & Payload ----------------------------------------------------------------------------------------------------
    ...
    def compartment_box1(self):
        return Box(
            height=0.1*self.w_c_root/0.4342,  # z
            length=self.fu_width*0.75,  # y
            width=self.w_c_root*0.35,  # x
            position=translate(self.position,
                               'x', self.w_c_root*0.25,
                               'z', 0.05*self.w_c_root/0.4342),
            centered=True,
            color="red",
            hidden=True)
