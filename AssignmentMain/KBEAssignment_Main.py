...
from math import tan, radians, pi, sqrt, sin, atan
...
# ATTRIBUTES ===========================================================================================================
    ...
    @Attribute
    def sweep_te(self):
        return atan((self.w_semi_span * tan(radians(self.sweep)) + self.w_c_tip - self.w_c_root) / self.w_semi_span)
...
# Propulsion system calculations ---------------------------------------------------------------------------------------
    ...
    @Attribute
    def p_c_root(self):
        return 0.018*self.w_c_root/0.4342
...
# Elevon calculations --------------------------------------------------------------------------------------------------
    ...
    @Attribute
    def position4(self):
        return rotate(self.position3,
                      'x', -radians(self.twist/4))

    @Attribute
    def position_winglet(self):
        return translate(
            rotate(self.position, "y", radians(self.twist)),
            "y", self.w_semi_span + self.fu_width / 2,
...
# PDF Report -----------------------------------------------------------------------------------------------------------
    ...
    def output_pdf(self):
        ...
        pdf.multi_cell(200, 4, "- Payload Weight        : " + str(self.payload) + " kg\n"
                               ...
                               "- LE Sweep Angle        : " + str(self.sweep) + " deg\n"
...
# PARTS ================================================================================================================
    ...
    def hinge_frame(self):
        return Frame(pos=self.position4,
                     hidden=True)
...
# Elevons --------------------------------------------------------------------------------------------------------------
    ...
    def hinge_cylinder(self):
        return Cylinder(
            ...
            angle=pi * 2,
            position=rotate(translate(self.position4,
                                      'x', -self.elevon_chord),
                            'x', radians(270)),
            hidden=True
        )
    ...
    def elevon_box1(self):
        return Box(width=self.w_c_tip,
                   ...
                   position=translate(self.position4,
                                      'x', -self.elevon_chord,
                                      'z', -self.hinge_radius),
                   hidden=True
                   )
    ...
    def elevon_box2(self):
        return Box(width=0.35 * self.w_semi_span,
                   length=0.35 * self.w_semi_span,
                   height=0.4 * self.w_semi_span,
                   position=translate(self.position,
                                      'x', self.w_c_root,
    ...
    def propeller_right(self):
        return LiftingSurface(
            ...
            position=rotate(translate  # longitudinal and vertically translation w.r.t. fuselage
                            (self.position,
                             "x", self.w_c_root + self.mot_t + 0.005,
                             "z", self.mot_r * 1.1 + self.p_c_root / 2),
                            "y", radians(65)),
            mesh_deflection=0.0001,
            color="gray",
            hidden=False,
...
# Winglets ------------------------------------------------------------------
    ...
    def right_winglet(self):
        return Winglet(
            ...
            xpos=self.w_semi_span*tan(radians(self.sweep))+self.w_c_tip/2,
    ...
    def left_winglet(self):
        return MirroredShape(
            shape_in=self.right_winglet.winglet,
            reference_point=self.position,
            vector1=self.position.Vz,
            vector2=self.position.Vx,
            mesh_deflection=0.0001,
